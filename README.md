...
LINE 群組 bot：只有群組裡有人 **@Grook**（或 bot 顯示名）時才回覆，整理「主人自己的」X（Twitter）貼文與轉發。

v1 **不做**每小時自動推播，也 **不包含**讚、回覆、瀏覽等互動數據。
...
- 內容：主人帳號的原創貼文 + 轉發。之後若要增刪回報欄位，改 `src/preferences` 即可。
...
src/
  ...
  summary/      時間窗、報表文字
  x/            XProvider 介面；mock 實作、api TODO
  store/        上次成功 @ 時間
